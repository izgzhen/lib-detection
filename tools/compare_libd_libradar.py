...
def main() :
    ...
    libd_dir = sys.argv[1]
    libradar_dir = sys.argv[2]
    # os.path.isdir
    # check to see if users pass in libd/output/ instead of libd/output
    assert os.path.isdir(libd_dir) and os.path.isdir(libradar_dir)

    file_names_libd = glob.glob(libd_dir + "/*.txt")
    file_names_libradar = glob.glob(libradar_dir + "/*.txt")

    # find the common apk files between two result folders
    file_names_libd = [os.path.basename(i) for i in file_names_libd]
    file_names_libradar = [os.path.basename(i) for i in file_names_libradar]
    ...
    for apk_sample in common_file_names:
        libd_res = get_library_libd(libd_dir + '/' + apk_sample.strip(), total_libraries)
        libradar_res = get_library_libRadar(libradar_dir + '/' + apk_sample.strip(), total_libraries)
